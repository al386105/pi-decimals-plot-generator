...
if __name__ == '__main__':
    ...
    for algorithm in data.keys():
        table_data = []
        for precision in data[algorithm].keys():
            if len(table_data) == 0:
                first_row = ["Precision"]
                first_row += list(data[algorithm][precision].keys())
                table_data.append(first_row)
            precision_row = [precision]
            precision_row += list(data[algorithm][precision].values())
            table_data.append(precision_row)

        file = open(path_to_save + f"EX-{algorithm}.tex", "w")
